"""
agents/synopsis_generator.py — Synopsis Generator Agent.

ПРИНЦИП: формулировки берутся ТОЧНО из шаблона синопсиса.
AI НЕ генерирует критерии включения/невключения.
Только подстановка значений + условная логика (пол, возраст, курение).

Порядок пунктов фиксирован. Если пункт не применим — он убирается,
остальные сдвигаются.
"""

from typing import Any, Dict, List
from app.agents.base import BaseAgent, AgentResult
import logging

logger = logging.getLogger(__name__)


# ════════════════════════════════════════════════════════
# ШАБЛОННЫЕ ТЕКСТЫ (точные формулировки из шаблона)
# ════════════════════════════════════════════════════════

def _build_inclusion_criteria(
    sex: str,
    age_min: int,
    age_max: int,
    inn: str,
) -> str:
    """
    Критерии включения — ТОЧНЫЙ текст из шаблона.
    Порядок фиксирован. Информированное согласие ВСЕГДА первое.
    """
    include_women = sex != "males_only"

    sex_text = "мужчины и женщины" if include_women else "мужчины"
    weight_text = (
        "более 45 кг для женщин и более 55 кг для мужчин и не более 110 кг"
        if include_women
        else "более 55 кг и не более 110 кг"
    )

    items: List[str] = []

    # 1. Информированное согласие — ВСЕГДА ПЕРВОЕ
    items.append(
        "Подписанная письменная форма информированного согласия до начала "
        "каких-либо процедур скрининга, а также способность, по мнению "
        "Исследователя, следовать всем требованиям Протокола."
    )

    # 2. Пол, возраст, курение
    items.append(
        f"Некурящие {sex_text} в возрасте от {age_min} до {age_max} лет "
        f"включительно на момент подписания формы Информированного согласия."
    )

    # 3. Здоров
    items.append(
        "Верифицированный диагноз «здоров» по данным стандартных клинических, "
        "лабораторных и инструментальных методов обследования."
    )

    # 4. ИМТ
    items.append(
        f"Индекс массы тела (ИМТ) от 18,5 до 30,0 кг/м² (включительно), "
        f"при массе тела {weight_text}."
    )

    # 5. АД
    items.append(
        "Уровень артериального давления (АД): систолическое артериальное "
        "давление (САД) от 100 до 129 мм рт. ст. (включительно), "
        "диастолическое артериальное давление (ДАД) от 60 до 89 мм рт. ст. "
        "(включительно)."
    )

    # 6. ЧСС
    items.append(
        "Частота сердечных сокращений (ЧСС) от 60 до 89 ударов в 1 минуту "
        "(включительно)."
    )

    # 7. ЧДД
    items.append(
        "Частота дыхательных движений (ЧДД) от 12 до 20 в 1 минуту "
        "(включительно)."
    )

    # 8. Температура
    items.append(
        "Температура тела от 36°C до 36,9°C (включительно)."
    )

    # 9. Алкоголь
    items.append(
        "Отказ от приема алкоголя в течение 72 часов до начала каких-либо "
        "процедур скрининга и в течение всего исследования."
    )

    # 10. Контрацепция
    contraception = (
        "Согласие добровольца на использование надежных методов контрацепции "
        "на протяжении всего исследования и в течение 1 месяца после его "
        "окончания. К адекватным методам контрацепции относятся: воздержание "
        "от половых контактов, кроме того, для мужчин: презерватив со "
        "спермицидом (пена, гель, крем)"
    )
    if include_women:
        contraception += (
            ", для женщин презерватив (для партнера), диафрагма или шеечный "
            "колпачок со спермицидом, внутриматочная спираль"
        )
    contraception += "."
    items.append(contraception)

    # Формируем текст
    header = (
        "Для включения в исследование добровольцы должны соответствовать "
        "всем следующим критериям включения:\n"
    )
    numbered = "\n".join(f"{i+1}. {item}" for i, item in enumerate(items))
    return header + numbered


def _build_exclusion_criteria(
    sex: str,
    inn: str,
    ref_drug: str,
) -> str:
    """
    Критерии невключения — ТОЧНЫЙ текст из шаблона.
    Пункты про женщин включаются только если sex != males_only.
    """
    include_women = sex != "males_only"

    items: List[str] = []

    items.append(
        "Наличие в анамнезе острых или хронических заболеваний "
        "сердечно-сосудистой, дыхательной, нервной, эндокринной, "
        "опорно-двигательной, кроветворной и иммунной систем, почек "
        "и мочевыводящих путей, печени и желчевыводящих путей, "
        "желудочно-кишечного тракта (ЖКТ), кожи, а также доброкачественных "
        "опухолевых и онкологических заболеваний."
    )

    items.append(
        "Хирургические вмешательства на ЖКТ в анамнезе "
        "(за исключением аппендэктомии не менее чем за 1 год до скрининга)."
    )

    items.append(
        "Заболевания/состояния, которые, по мнению исследователя, могут "
        "повлиять на абсорбцию, распределение, метаболизм или экскрецию "
        "исследуемых лекарственных препаратов."
    )

    items.append(
        "Острые инфекционные заболевания менее чем за 30 дней до скрининга."
    )

    items.append(
        "Регулярный или разовый прием любых лекарственных препаратов "
        "(включая витамины, препараты растительного происхождения "
        "и биологически активные добавки) менее чем за 30 дней до скрининга."
    )

    items.append(
        "Донорство (> 450 мл крови или плазмы) менее чем за два месяца "
        "до скрининга."
    )

    if include_women:
        items.append(
            "Применение гормональных контрацептивов (у женщин) "
            "менее чем за 2 месяца до скрининга."
        )

    items.append(
        "Применение депо-инъекций или имплантация любых препаратов "
        "в течение 6 месяцев до скрининга."
    )

    items.append(
        "Любые отклонения от нормальных значений при лабораторном "
        "и/или инструментальном обследовании на скрининге."
    )

    items.append(
        "Положительный тест на содержание алкоголя в выдыхаемом воздухе."
    )

    items.append(
        "Положительный тест на содержание наркотических "
        "и сильнодействующих лекарственных веществ в моче."
    )

    if include_women:
        items.append(
            "Положительный тест на беременность (по результатам тест полосок) "
            "для женщин с сохраненным репродуктивным потенциалом."
        )

    items.append("Положительный тест мочи на котинин.")

    items.append(
        "Положительные результаты тестов на маркеры гепатита В (HBsAg), "
        "С (HCV), сифилиса, ВИЧ-инфекцию."
    )

    items.append(
        "Высокая вероятность возникновения проблем с успешной установкой "
        "венозного катетера или с выполнением пункции вены на предплечье."
    )

    items.append(
        "Повышенная чувствительность к гепарину или "
        "гепарин-индуцированная тромбоцитопения в анамнезе."
    )

    items.append(
        "Участие в другом клиническом исследовании менее чем за 3 месяца "
        "перед скринингом и/или параллельно с настоящим исследованием."
    )

    items.append(
        f"Гиперчувствительность к {inn}, или к любому другому компоненту, "
        f"входящему в состав исследуемых лекарственных препаратов в анамнезе."
    )

    items.append(
        "Отягощенный аллергологический анамнез, лекарственная непереносимость."
    )

    items.append(
        "Непереносимость лактозы, дефицит лактазы или "
        "глюкозо-галактозная мальабсорбция."
    )

    items.append(
        "Прием более чем 10 ед. алкоголя в неделю в анамнезе "
        "(где каждая единица алкоголя эквивалентна 500 мл пива, "
        "200 мл вина или 50 мл крепких спиртных напитков) "
        "и/или анамнестические сведения об алкоголизме, наркомании, "
        "злоупотреблении лекарственными препаратами."
    )

    items.append(
        "Дегидратация из-за диареи, рвоты или другой причины "
        "в течение последних 24 часов до приема исследуемого препарата."
    )

    items.append(
        "Планирование пребывания в стационаре в период проведения "
        "исследования, по какому-либо поводу, кроме госпитализации, "
        "предусмотренной данным протоколом."
    )

    items.append(
        "Наличие неустойчивой структуры сна (например, работа в ночную смену, "
        "нарушения сна, бессонница, недавнее возвращение из другого часового "
        "пояса и т.п.), экстремальные физические нагрузки "
        "(например, поднятие тяжестей)."
    )

    items.append(
        "Особая диета (например, вегетарианская, веганская, гипокалорийная "
        "(менее 1000 ккал/сут)) в течение 30 дней до скрининга "
        "и до завершения участия в исследовании."
    )

    items.append(
        "Употребление следующих напитков и продуктов питания: "
        "содержащих производные ксантина (шоколад, кофе, чай, газированные "
        "напитки, содержащие кофеин, энергетические напитки) за 72 ч до приема "
        "исследуемых препаратов; содержащих ингибиторы или индукторы ферментных "
        "систем организма (грейпфрут, помело, зверобой продырявленный, клюква) "
        "за 7 дней до приема исследуемых препаратов."
    )

    if include_women:
        items.append(
            "Беременные или кормящие женщины или женщины, планирующие "
            "беременность во время клинического исследования; женщины "
            "с сохраненным репродуктивным потенциалом (в том числе не "
            "стерилизованные хирургическим путем и в периоде постменопаузы "
            "менее 2 лет), не использующие адекватных методов контрацепции."
        )
        items.append(
            "Женщины с сохраненным репродуктивным потенциалом, имеющие "
            "незащищенный половой акт с нестерилизованным партнером "
            "мужского пола в течение 30 дней до скрининга."
        )

    items.append(
        "Неспособность читать или писать; нежелание понять и следовать "
        "процедурам протокола исследования; невыполнение процедур, которое, "
        "по мнению Исследователя, может повлиять на результаты исследования "
        "или безопасность добровольца и препятствовать дальнейшему участию "
        "добровольца в исследовании; любые другие сопутствующие медицинские "
        "или серьезные психические состояния, которые делают добровольца "
        "непригодным для участия в клиническом исследовании, ограничивают "
        "правомерность получения информированного согласия или могут повлиять "
        "на способность добровольца принять участие в исследовании."
    )

    items.append(
        f"Наличие по мнению Исследователя значимого риска или противопоказания "
        f"к применению препарата {ref_drug}, согласно инструкции "
        f"по применению данного препарата."
    )

    header = (
        "Добровольцы не будут включены в исследование, "
        "если присутствует любой из следующих критериев:\n"
    )
    numbered = "\n".join(f"{i+1}. {item}" for i, item in enumerate(items))
    return header + numbered


# ════════════════════════════════════════════════════════
# АГЕНТ
# ════════════════════════════════════════════════════════

class SynopsisGeneratorAgent(BaseAgent):
    """
    Synopsis Generator — финальный агент.
    Использует ТОЧНЫЕ формулировки из шаблона. LLM только для литобзора.
    """

    async def run(self, input_data: Dict[str, Any]) -> AgentResult:
        # ── Распаковка ──
        inn_ru = input_data.get("inn_ru", "")
        dosage_form = input_data.get("dosage_form", "")
        dosage = input_data.get("dosage", "")
        release_type = input_data.get("release_type", "immediate")
        drug_name_trade = input_data.get("drug_name_trade", "")
        reference_drug_name = input_data.get("reference_drug_name", "")
        sponsor = input_data.get("sponsor_name", "________")
        center = input_data.get("research_center", "________")
        lab = input_data.get("bioanalytical_lab", "________")
        insurance = input_data.get("insurance_company", "________")
        sex = input_data.get("sex_restriction", "males_only")
        age_min = input_data.get("age_min", 18)
        age_max = input_data.get("age_max", 45)
        smoking = input_data.get("smoking_restriction", "non_smokers")

        pk = input_data.get("pk", {})
        if hasattr(pk, "model_dump"):
            pk = pk.model_dump()

        design = input_data.get("design", {})
        if hasattr(design, "model_dump"):
            design = design.model_dump()

        sample = input_data.get("sample_size", {})
        if hasattr(sample, "model_dump"):
            sample = sample.model_dump()

        ref_drug = pk.get("reference_drug", reference_drug_name) or "________"
        intake_mode = design.get("intake_mode", "fasting")
        # Запоминаем, задал ли пользователь intake_mode явно через CLI
        intake_mode_input = input_data.get("intake_mode") is not None
        n_total = sample.get("n_total", 24)

        # ── Текст дизайна для названия протокола ──
        design_type = design.get("design_type", "2x2_crossover")
        if hasattr(design_type, "value"):
            design_type = design_type.value
        n_periods = int(design.get("n_periods", 2) or 2)
        n_sequences = int(design.get("n_sequences", 2) or 2)

        _periods_word = {1: "одном", 2: "двух", 3: "трёх", 4: "четырёх"}.get(n_periods, str(n_periods))
        _seq_word = {2: "двух", 3: "трёх", 4: "четырёх"}.get(n_sequences, str(n_sequences))

        # Маппинг дизайна для названия протокола (по спецификации)
        _title_map = {
            # Простой перекрёстный
            "2x2_crossover": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях",
            "3x3_crossover": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях",
            # Частично репликативный
            "partial_replicate": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях с частично репликативной схемой",
            "replicate_3_period": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях с частично репликативной схемой",
            # Полный репликативный
            "full_replicate_3_period": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях с полной репликативной схемой",
            "full_replicate_4_period": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях с полной репликативной схемой",
            "replicate_4_period": f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях с полной репликативной схемой",
            # Параллельный
            "parallel_1_period": "в параллельных группах",
            "parallel_2_period": f"в параллельных группах в {_periods_word} периодах",
            "parallel": "в параллельных группах",
            # Адаптивный
            "adaptive_crossover": f"двухэтапное перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях адаптивное",
            "adaptive_parallel_1": "двухэтапное в параллельных группах адаптивное",
            "adaptive_parallel_2": f"двухэтапное в параллельных группах в {_periods_word} периодах адаптивное",
        }
        design_title_text = _title_map.get(
            design_type,
            f"перекрёстное в {_periods_word} периодах и {_seq_word} последовательностях"
        )

        # ── Текст приёма ──
        if hasattr(intake_mode, "value"):
            intake_mode = intake_mode.value
        intake_text_short = {
            "fasting": "натощак",
            "fed": "после приема высококалорийной пищи",
            "both": "натощак и после приема высококалорийной пищи",
        }.get(intake_mode, "натощак")

        # ── Текст исследуемого препарата ──
        # Формат: Название, лекарственная форма дозировка (Производитель, Страна)
        mfr_name = input_data.get("manufacturer_name", "")
        mfr_country = input_data.get("manufacturer_country", "")
        test_drug_short = f"{drug_name_trade or inn_ru}, {dosage_form}, {dosage}"
        if mfr_name:
            mfr_str = f"{mfr_name}, {mfr_country}" if mfr_country else mfr_name
            test_drug_full = f"{drug_name_trade or inn_ru}, {dosage_form} {dosage} ({mfr_str})"
        else:
            test_drug_full = test_drug_short

        # ── Текст референтного препарата ──
        # Формат: Название, лекарственная форма дозировка (Производитель, Страна)
        # Данные заполнятся после fetch_drug_info, пока используем заглушки
        ref_drug_title = f"{ref_drug}, {dosage_form} {dosage}"

        # ── Пол для названия ──
        # "женского пола" добавляется ТОЛЬКО если females_only
        sex_title_text = ""  # заполнится после fetch_drug_info

        # ── Построение синопсиса ──
        synopsis = {}

        # protocol_title будет сформирован после fetch_drug_info (ниже)
        synopsis["sponsor"] = sponsor
        synopsis["research_center"] = center
        synopsis["bioanalytical_lab"] = lab
        synopsis["insurance_company"] = insurance
        synopsis["test_drug"] = test_drug_short
        synopsis["reference_drug"] = ref_drug
        synopsis["inn"] = inn_ru
        synopsis["dosage_form"] = dosage_form
        synopsis["dosage"] = dosage
        synopsis["n_subjects"] = str(n_total)
        synopsis["protocol_version"] = "1.0"
        synopsis["study_id"] = input_data.get("study_id", "")
        synopsis["manufacturer"] = input_data.get("manufacturer_name", "")
        synopsis["manufacturer_country"] = input_data.get("manufacturer_country", "")
        synopsis["storage_conditions"] = input_data.get("storage_conditions", "")
        synopsis["excipients"] = input_data.get("excipients", "")
        synopsis["composition"] = input_data.get("composition", "")

        # ── Информация из инструкции к референтному препарату ──
        # Используем drug_info, уже загруженный в PK Agent (один парсинг!)
        _drug_info_from_pk = input_data.get("drug_info")
        if _drug_info_from_pk:
            try:
                try:
                    from app.utils.drug_info_parser import drug_info_to_dict
                except ImportError:
                    from drug_info_parser import drug_info_to_dict
                ref_dict = drug_info_to_dict(_drug_info_from_pk)
                synopsis.update(ref_dict)
                if _drug_info_from_pk.source_url:
                    logger.info(f"✅ Инструкция к {ref_drug} (из PK Agent): {_drug_info_from_pk.source_url}")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка drug_info_to_dict: {e}")
        else:
            # Fallback: если PK Agent не вернул drug_info — загружаем
            try:
                try:
                    from app.utils.drug_info_parser import fetch_drug_info, drug_info_to_dict
                except ImportError:
                    try:
                        from drug_info_parser import fetch_drug_info, drug_info_to_dict
                    except ImportError:
                        fetch_drug_info = None

                _drug_name_for_fetch = reference_drug_name or ref_drug
                if fetch_drug_info and _drug_name_for_fetch and _drug_name_for_fetch != "________":
                    ref_info = await fetch_drug_info(
                        drug_name=_drug_name_for_fetch,
                        inn=inn_ru,
                        dosage=dosage,
                    )
                    ref_dict = drug_info_to_dict(ref_info)
                    synopsis.update(ref_dict)
                    if ref_info.source_url:
                        logger.info(f"✅ Инструкция к {_drug_name_for_fetch} (fallback): {ref_info.source_url}")
            except Exception as e:
                logger.warning(f"⚠️ Не удалось загрузить инструкцию: {e}")

        # ── Информация об организациях (адрес, телефон) ──
        # Ищем полные данные для: спонсор, центр, лаборатория, страховая
        try:
            from app.services.search.yandex_search import (
                search_organization_info, format_sponsor_field,
            )
            _has_org_search = True
        except ImportError:
            try:
                from yandex_search import search_organization_info, format_sponsor_field
                _has_org_search = True
            except ImportError:
                _has_org_search = False

        if not _has_org_search:
            print("  ⚠️ yandex_search модуль не найден — поиск организаций отключён")
        else:
            print(f"  🔎 Поиск организаций: спонсор={sponsor}, центр={center}, лаб={lab}, страховая={insurance}")

            import time
            _org_query_count = 0

            def _org_search_with_delay(name: str, country: str = "Россия"):
                """Поиск с паузой 1.5 сек между запросами (Yandex rate limit: 1 req/sec)."""
                nonlocal _org_query_count
                if _org_query_count > 0:
                    time.sleep(1.5)
                _org_query_count += 1
                return search_organization_info(name, country)

        if _has_org_search:
            import time as _time
            sponsor_country = input_data.get("sponsor_country", "Россия")

            # Спонсор
            if sponsor and sponsor != "________":
                try:
                    org_info = _org_search_with_delay(sponsor, sponsor_country)
                    if org_info.get("address") or org_info.get("phone"):
                        sponsor_full = format_sponsor_field(org_info)
                        synopsis["sponsor"] = sponsor_full
                        print(f"  ✅ Спонсор найден: {org_info.get('address', '')}")
                    else:
                        print(f"  ⚠️ Спонсор: Yandex не вернул адрес/телефон, оставляем название")
                except Exception as e:
                    print(f"  ⚠️ Спонсор ошибка: {e}")
                    logger.warning(f"⚠️ Спонсор: {e}")

            # Пауза между запросами к Yandex GenSearch
            _time.sleep(1)

            # Исследовательский центр
            if center and center != "________":
                try:
                    center_info = _org_search_with_delay(center, "Россия")
                    if center_info.get("address") or center_info.get("phone"):
                        center_full = format_sponsor_field(center_info)
                        synopsis["research_center"] = center_full
                        print(f"  ✅ Центр найден: {center_info.get('address', '')}")
                    else:
                        print(f"  ⚠️ Центр: Yandex не вернул адрес/телефон, оставляем название")
                except Exception as e:
                    print(f"  ⚠️ Центр ошибка: {e}")
                    logger.warning(f"⚠️ Центр: {e}")

            _time.sleep(1)

            # Биоаналитическая лаборатория
            if lab and lab != "________":
                try:
                    # Если лаборатория = центр — не ищем повторно
                    if lab == center and synopsis.get("research_center"):
                        synopsis["bioanalytical_lab"] = synopsis["research_center"]
                        print(f"  ✅ Лаборатория = Центр (пропускаем повторный поиск)")
                    else:
                        lab_info = _org_search_with_delay(lab, "Россия")
                        if lab_info.get("address") or lab_info.get("phone"):
                            lab_full = format_sponsor_field(lab_info)
                            synopsis["bioanalytical_lab"] = lab_full
                            print(f"  ✅ Лаборатория найдена: {lab_info.get('address', '')}")
                        else:
                            print(f"  ⚠️ Лаборатория: Yandex не вернул адрес/телефон, оставляем название")
                except Exception as e:
                    print(f"  ⚠️ Лаборатория ошибка: {e}")
                    logger.warning(f"⚠️ Лаборатория: {e}")

            _time.sleep(1)

            # Страховая компания
            if insurance and insurance != "________":
                try:
                    ins_info = _org_search_with_delay(insurance, "Россия")
                    if ins_info.get("address") or ins_info.get("phone"):
                        ins_full = format_sponsor_field(ins_info)
                        synopsis["insurance_company"] = ins_full
                        print(f"  ✅ Страховая найдена: {ins_info.get('address', '')}")
                    else:
                        print(f"  ⚠️ Страховая: Yandex не вернул адрес/телефон, оставляем название")
                except Exception as e:
                    print(f"  ⚠️ Страховая ошибка: {e}")
                    logger.warning(f"⚠️ Страховая: {e}")

        # Критерии — ТОЧНЫЙ текст из шаблона
        # Если парсер инструкции определил что препарат только для женщин —
        # переключаем пол добровольцев
        if synopsis.get("suggested_sex") and synopsis["suggested_sex"] != "males_only":
            sex = synopsis["suggested_sex"]
            logger.info(f"ℹ️ Пол добровольцев из инструкции к {ref_drug}: {sex}")

        # Если парсер инструкции определил режим приёма (натощак/после еды) —
        # переключаем intake_mode и пересчитываем текст
        if synopsis.get("suggested_intake") and not intake_mode_input:
            _suggested = synopsis["suggested_intake"]
            if _suggested in ("fasting", "fed", "both"):
                intake_mode = _suggested
                intake_text_short = {
                    "fasting": "натощак",
                    "fed": "после приема высококалорийной пищи",
                    "both": "натощак и после приема высококалорийной пищи",
                }.get(intake_mode, "натощак")
                logger.info(f"ℹ️ Режим приёма из инструкции к {ref_drug}: {_suggested}")
        elif not intake_mode_input and _drug_name_for_fetch:
            # Fallback: ищем режим приёма через Yandex Search
            try:
                from app.services.search.yandex_search import search_intake_mode
            except ImportError:
                try:
                    from yandex_search import search_intake_mode
                except ImportError:
                    search_intake_mode = None
            if search_intake_mode:
                try:
                    _intake_result = search_intake_mode(
                        ref_drug_name=_drug_name_for_fetch,
                        inn_ru=inn_ru,
                    )
                    _mode = _intake_result.get("mode", "")
                    if _mode in ("fasting", "fed", "both"):
                        intake_mode = _mode
                        intake_text_short = {
                            "fasting": "натощак",
                            "fed": "после приема высококалорийной пищи",
                            "both": "натощак и после приема высококалорийной пищи",
                        }.get(intake_mode, "натощак")
                        logger.info(f"ℹ️ Режим приёма (Yandex Search) для {_drug_name_for_fetch}: {_mode}")
                except Exception as e:
                    logger.debug(f"Yandex intake search failed: {e}")

        # ── Формируем protocol_title (после fetch, когда известны все данные) ──
        # Производитель референта из инструкции — переводим на русский
        ref_mfr = synopsis.get("ref_manufacturer", "")
        if ref_mfr:
            # Переводим EN → RU если есть латиница
            _en_to_ru = None
            try:
                from app.utils.inn_utils import ensure_russian_text as _en_to_ru
            except ImportError:
                try:
                    from inn_utils import ensure_russian_text as _en_to_ru
                except ImportError:
                    pass
            if _en_to_ru:
                ref_mfr_ru = _en_to_ru(ref_mfr)
                if ref_mfr_ru != ref_mfr:
                    print(f"  🌐 ref_manufacturer: '{ref_mfr}' → '{ref_mfr_ru}'")
                    ref_mfr = ref_mfr_ru
                    synopsis["ref_manufacturer"] = ref_mfr
            ref_drug_title = f"{reference_drug_name or ref_drug}, {dosage_form} {dosage} ({ref_mfr})"
        else:
            ref_drug_title = f"{reference_drug_name or ref_drug}, {dosage_form} {dosage}"

        # Пол: "женского пола" только для females_only
        if sex == "females_only":
            sex_title_text = " женского пола"
        else:
            sex_title_text = ""

        synopsis["protocol_title"] = (
            f"Открытое рандомизированное {design_title_text} исследование "
            f"сравнительной фармакокинетики и биоэквивалентности "
            f"препаратов {test_drug_full} и {ref_drug_title} "
            f"у здоровых добровольцев{sex_title_text} {intake_text_short}"
        )

        # Сохраняем intake_mode для docx_exporter
        synopsis["intake_mode"] = intake_mode

        # ── Текст «Дизайн исследования» ──
        # Формирует описательный абзац по эталону iFarma
        hvd_comp_ru = input_data.get("hvd_component_ru", "")
        hvd_comp_en = input_data.get("hvd_component_en", "")
        hvd_cv = input_data.get("hvd_cv_value")
        cv_max = pk.get("cv_intra_max")
        is_hvd = pk.get("is_hvd", False)
        t_half = pk.get("t_half_hours")
        washout_days = design.get("washout_days") or design.get("washout_min_days")

        # Вычисляем отмывочный период как 5×T½ если не указан
        if t_half and not washout_days:
            washout_days = max(7, round(5 * t_half / 24) + 1)  # дней, минимум 7

        study_id = input_data.get("study_id", "")

        if is_hvd and "replicate" in design_type:
            # HVD → репликативный дизайн с обоснованием
            hvd_name = hvd_comp_ru or inn_ru
            cv_text = f"{hvd_cv or cv_max:.0f}%" if (hvd_cv or cv_max) else "более 30%"

            # Определяем текст компонента
            if hvd_comp_ru and "+" in inn_ru:
                # Комбинация — указываем какой компонент HVD
                component_phrase = (
                    f"Один из компонентов исследуемого препарата, {hvd_name}, "
                    f"является высоковариабельным "
                    f"(максимальная концентрация в плазме крови (Сmax) "
                    f"составляет приблизительно {cv_text} [1])."
                )
            else:
                # Одиночный МНН
                component_phrase = (
                    f"Исследуемый препарат является высоковариабельным "
                    f"(внутрииндивидуальная вариабельность Сmax "
                    f"составляет приблизительно {cv_text} [1])."
                )

            synopsis["study_design_description"] = (
                f"{component_phrase} "
                f"Согласно действующим Правилам Евразийского экономического союза (ЕАЭС) "
                f"для сравнительной оценки фармакокинетических параметров препаратов "
                f"с высокой вариабельностью "
                f"{test_drug_full} и {ref_drug_title} "
                f"будет проведено исследование биоэквивалентности "
                f"с повторным (репликативным) перекрестным дизайном "
                f"в {_periods_word} периодах и в {_seq_word} группах "
                f"с приемом однократной дозы. "
                f"При этом периоды приема препаратов исследования будут разделены "
                f"отмывочным периодом продолжительностью не менее пяти периодов "
                f"полувыведения (Т1/2) действующих веществ."
            )
        else:
            # Стандартный дизайн
            synopsis["study_design_description"] = (
                f"Для сравнительной оценки фармакокинетических параметров препаратов "
                f"{test_drug_full} и {ref_drug_title} "
                f"будет проведено исследование биоэквивалентности "
                f"с {design_title_text} дизайном "
                f"с приемом однократной дозы. "
                f"При этом периоды приема препаратов исследования будут разделены "
                f"отмывочным периодом продолжительностью не менее пяти периодов "
                f"полувыведения (Т1/2) действующих веществ."
            )

        synopsis["inclusion_criteria"] = _build_inclusion_criteria(
            sex=sex, age_min=age_min, age_max=age_max, inn=inn_ru,
        )
        synopsis["exclusion_criteria"] = _build_exclusion_criteria(
            sex=sex, inn=inn_ru, ref_drug=ref_drug,
        )

        # Критерии БЭ
        be_lower = design.get("be_lower", 80.0)
        be_upper = design.get("be_upper", 125.0)
        synopsis["be_criteria"] = (
            f"Вывод о биоэквивалентности сравниваемых препаратов будет сделан "
            f"с использованием подхода, основанного на оценке 90% доверительных "
            f"интервалов для отношений средних геометрических значений "
            f"исследуемого препарата (T) к препарату сравнения (R) для "
            f"фармакокинетических параметров AUC0-t и Cmax {inn_ru}. "
            f"Препараты считаются биоэквивалентными, если границы оцененных "
            f"доверительных интервалов для AUC0-t и Cmax находятся в пределах "
            f"{be_lower:.2f}-{be_upper:.2f}% (α=0.05) для изучаемого аналита."
        )

        # ── Список источников ──
        # Собираем из PK-источников + нормативные документы
        sources_list = []
        pk_sources = pk.get("sources", [])
        if isinstance(pk_sources, list):
            for src in pk_sources:
                if isinstance(src, dict):
                    src_type = src.get("source_type", "")
                    title_text = src.get("title", "")
                    pmid = src.get("pmid", "")
                    url = src.get("url", "")
                    doi = src.get("doi", "")
                    parts = []
                    if title_text:
                        parts.append(title_text)
                    if pmid:
                        parts.append(f"PMID: {pmid}")
                    if doi:
                        parts.append(f"DOI: {doi}")
                    if url:
                        parts.append(url)
                    if parts:
                        sources_list.append(". ".join(parts))

        # Нормативные документы — всегда включаем
        sources_list.extend([
            "Решение Совета Евразийской экономической комиссии от 03.11.2016 № 85 "
            "«Об утверждении Правил проведения исследований биоэквивалентности "
            "лекарственных препаратов в рамках Евразийского экономического союза»",
            "Решение Совета Евразийской экономической комиссии от 03.11.2016 № 79 "
            "«Об утверждении Правил Надлежащей клинической практики "
            "Евразийского экономического союза»",
            "ГОСТ Р 57679-2017 «Лекарственные средства. "
            "Исследование биоэквивалентности лекарственных препаратов»",
        ])

        # Литературный обзор — если есть, упоминаем
        lit_review = pk.get("literature_review", "")
        if lit_review and "ОХЛП" in lit_review:
            ref_name = pk.get("reference_drug", ref_drug)
            if ref_name and ref_name != "________":
                sources_list.insert(0, f"Общая характеристика лекарственного препарата (ОХЛП) {ref_name}")

        synopsis["sources_text"] = "\n".join(
            f"{i+1}. {s}" for i, s in enumerate(sources_list)
        )

        # ── Пост-обработка: перевод EN → RU ──
        # Все поля синопсиса должны быть на русском языке.
        # Переводим через Yandex Translate любые поля с латиницей.
        _translate_fn = None
        try:
            from app.utils.inn_utils import ensure_russian_text as _translate_fn
        except ImportError:
            try:
                from inn_utils import ensure_russian_text as _translate_fn
            except ImportError:
                pass

        if _translate_fn:
            # Поля, которые должны быть на русском в документе
            _fields_to_translate = [
                "ref_manufacturer",
                "manufacturer",
                "manufacturer_country",
                "reference_drug",
                "protocol_title",
                "study_design_description",
            ]
            for fld in _fields_to_translate:
                val = synopsis.get(fld, "")
                if val and isinstance(val, str):
                    translated = _translate_fn(val)
                    if translated != val:
                        print(f"  🌐 {fld}: '{val}' → '{translated}'")
                        synopsis[fld] = translated

        return AgentResult(data=synopsis, sources=["synopsis_template", "decision_85"])